...
void HttpServer::incomingConnection(qintptr socketDescriptor) {
    ...
    qDebug() << "Новое подлкючение с дескриптором: "
             << socket->socketDescriptor();

    // connect(socket, &QTcpSocket::readyRead, this,
    // &HttpServer::handleReadyRead);
...
void HttpServer::processLogin(QTcpSocket *socket, const QJsonObject &obj) {
    ...
    QSqlQuery query(db);
    query.prepare("SELECT COUNT(*) FROM users WHERE "
                  "username = :username AND "
                  "password_hash "
                  "= :password");
    query.bindValue(":username", login);
    query.bindValue(":password",
        QString(passwordHash)); // передаём хеш в строковом виде
...
void HttpServer::processRegister(QTcpSocket *socket, const QJsonObject &obj) {
    ...
    checkQuery.prepare("SELECT COUNT(*) FROM users WHERE "
                       "username = :username");
    ...
    insertQuery.prepare("INSERT INTO users (username, "
...
void HttpServer::sendHttpResponse(QTcpSocket *socket,
...
QString getConfigPath() {
    ...
    QString configPath =
        QCoreApplication::applicationDirPath() + "/db_config.json";
...
bool HttpServer::connectToDatabase() {
    ...
    if (port == 0) {
        qDebug() << "Порт из конфигурации равен 0, "
                    "возможно ошибка в файле";
        return false;
    }
