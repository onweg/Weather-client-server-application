...
void HttpServer::incomingConnection(qintptr socketDescriptor) {
  ...
  connect(socket, &QTcpSocket::readyRead, this, [=]() mutable {
    ...
    if (buffer.size() < totalSize) return;  // ждём остальное
...
void HttpServer::processLogin(QTcpSocket *socket, const QJsonObject &obj) {
  ...
  QSqlQuery query(db);
  query.prepare(
      "SELECT COUNT(*) FROM users WHERE username = :username AND password_hash "
      "= :password");
...
void HttpServer::sendHttpResponse(QTcpSocket *socket,
                                  const QJsonObject &response, int statusCode) {
