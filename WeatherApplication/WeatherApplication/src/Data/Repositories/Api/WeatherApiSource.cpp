...
void WeatherApiSource::findWeatherDataByCity(
 ...
 QString url =
	 urlTemplate.arg(cityName, QString::fromStdString(apiConfig_.value().key));
 ...
 connect(
	 reply, &QNetworkReply::finished, this, [this, reply, callback, city]() {
	  ...
	  if (error.error != QJsonParseError::NoError) {
	   callback(Result<WeekWeatherData>::failure(
		   "Ошибка парсинга JSON: " + error.errorString().toStdString()));
	   reply->deleteLater();
	   return;
	  }
	  ...
	  connect(weatherReply, &QNetworkReply::finished, this,
			  [weatherReply, callback]() {
			   ...
			   if (!weatherDoc.isObject()) {
				callback(Result<WeekWeatherData>::failure(
					"JSON с погодой не является объектом"));
				weatherReply->deleteLater();
				return;
			   }

			   QJsonObject json = weatherDoc.object();
			   WeekWeatherData tmpResult =
			   ...
			   if (tmpResult.messageError.empty()) {
				...
				result =
					Result<WeekWeatherData>::failure(tmpResult.messageError);
			   }
