...
public:
 UserRepository(std::shared_ptr<IConfigProvider> config,
				std::shared_ptr<ISharedState> state, QObject* parent = nullptr);
 void findUser(const User& user,
			   std::function<void(Result<User>)> callback) override;
 void registerUser(const User& user,
				   std::function<void(Result<User>)> callback) override;

private:
 void sendRequest(const User& user, std::function<void(Result<User>)> callback,
