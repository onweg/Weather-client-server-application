...
UserRepository::UserRepository(std::shared_ptr<IConfigProvider> config,
 ...
 if (configProvider_) {
  ...
  serverHostConfig_ =
	  Result<ServerHostConfig>::failure("Ну удалось получить IConfigProvider");
...
void UserRepository::sendRequest(const User &user,
 ...
 if (serverHostConfig_.isSuccess()) {
  QString urlStr = "http://" +
				   QString::fromStdString(serverHostConfig_.value().ip) + ":" +
  ...
  if (command == "LOGIN") {
   data =
	   AuthorizationInfoJsonConverter::loginUserToJsonDocument(user).toJson();
