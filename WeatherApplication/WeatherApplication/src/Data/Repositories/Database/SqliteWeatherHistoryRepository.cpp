...
std::vector<WeatherHistoryItem> SqliteWeatherHistoryRepository::getAll() {
  ...
  QSqlQuery query(
      "SELECT id, username, timestamp, city, date FROM weather_data");
  ...
  while (query.next()) {
    ...
    row.timestamp = std::chrono::system_clock::from_time_t(
        static_cast<std::time_t>(timestamp.toMSecsSinceEpoch() / 1000));
...
void SqliteWeatherHistoryRepository::save(const std::string &city,
  ...
  QSqlQuery query;
  query.prepare(
      "INSERT INTO weather_data (username, city, date) "
