...
SqliteWeatherHistoryRepository::SqliteWeatherHistoryRepository(
    std::shared_ptr<IWeatherDatabaseInitializer> dbInit,
...
{
	QSqlQuery query(db_);
	query.prepare("INSERT INTO weather_data (username, city, date) VALUES "
	              "(:username, :city, :date)");
...
void SqliteWeatherHistoryRepository::bindInsertValues(QSqlQuery &query,
                                                      const std::string &city,
                                                      const std::string &date)
{
	query.bindValue(":username",
	                QString::fromStdString(sharedState_->getUsername()));
...
std::vector<WeatherHistoryItemDto>
...
SqliteWeatherHistoryRepository::mapRowToDto(QSqlQuery &query)
{
	...
	dto.timestamp =
	    std::chrono::system_clock::from_time_t(msecs / 1000); // в секундах
	return dto;
}
