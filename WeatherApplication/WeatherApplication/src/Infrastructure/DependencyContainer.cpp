...
DependencyContainer::DependencyContainer(QObject* qmlRoot) : qmlRoot_(qmlRoot) {
 ...
 auto configProviderImpl =
	 std::make_shared<ConfigProvider>(configLoaderInterface_);
 configProviderInterface_ = configProviderImpl;
 auto* userRepositoryImpl = new UserRepository(configProviderInterface_,
											   sharedStateInterface_, qmlRoot_);
 userRepositoryInterface_ = userRepositoryImpl;
 auto* weatherApiSourceImpl =
	 new WeatherApiSource(configProviderInterface_, qmlRoot);
 apiWeatherSourceInterface_ = weatherApiSourceImpl;
 auto dbWeatherHistoryImpl = std::make_shared<SqliteWeatherHistoryRepository>(
 ...
 dailyWeatherUseCase_ = std::make_shared<GetDailyWeatherUseCase>(
	 apiWeatherSourceInterface_, cacheSourceInterface_);
 ...
 weatherViewModel = new WeatherViewModel(
	 dailyWeatherUseCase_, weeklyWeatherUseCase_, saveHistoryUseCase_, qmlRoot);
}
