...
GetWeeklyWeatherUseCase::GetWeeklyWeatherUseCase(IApiWeatherSource *apiRepo,
    std::shared_ptr<IWeatherCacheSource> cacheRepo)
    : apiRepository_(apiRepo), cacheRepository_(std::move(cacheRepo)) {
}

void GetWeeklyWeatherUseCase::execute(const std::string &city,
    ...
    apiRepository_->findWeatherDataByCity(
        city, [this, city, callback](Result<WeekWeatherData> apiResult) {
            if (!apiResult.isSuccess()) {
                callback(
                    Result<WeekWeatherData>::failure(apiResult.errorMessage()));
                return;
            }
