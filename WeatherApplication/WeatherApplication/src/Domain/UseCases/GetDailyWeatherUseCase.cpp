...
GetDailyWeatherUseCase::GetDailyWeatherUseCase(IApiWeatherSource *apiRepo,
    std::shared_ptr<IWeatherCacheSource> cacheRepo)
    : apiRepository_(apiRepo), cacheRepository_(std::move(cacheRepo)) {
}

void GetDailyWeatherUseCase::execute(const std::string &city,
    ...
    apiRepository_->findWeatherDataByCity(
        city, [this, city, date, callback](Result<WeekWeatherData> weekResult) {
            if (!weekResult.isSuccess()) {
                callback(
                    Result<WeatherData>::failure(weekResult.errorMessage()));
                return;
            }
            ...
            if (updatedResult.isSuccess()) {
                ...
                callback(Result<WeatherData>::failure(
                    "Погода на дату " + date +
                    " не найдена даже после обновления"));
            }
        });
}
