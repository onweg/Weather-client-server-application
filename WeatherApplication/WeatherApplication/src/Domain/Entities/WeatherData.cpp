...
WeatherData::WeatherData()
 : m_lat(0.0), m_lon(0.0), m_city("..."), m_description("..."), m_temp(0.0),
   m_feels_like(0.0), m_temp_max(0.0), m_temp_min(0.0), m_wind_speed(0.0),
   m_humidity(0), m_pressure(0)
{
}

WeatherData::WeatherData(double lat, double lon, const std::string &city,
                         const std::string &date,
                         const std::string &description, double temp,
                         double feels_like, double temp_max, double temp_min,
                         double wind_speed, int humidity, int pressure,
                         const std::string &messageError)
 : m_lat(lat), m_lon(lon), m_city(city), m_date(date),
   m_description(description), m_temp(temp), m_feels_like(feels_like),
   m_temp_max(temp_max), m_temp_min(temp_min), m_wind_speed(wind_speed),
   m_humidity(humidity), m_pressure(pressure), m_messageError(messageError)
{
}
...
{
	return m_city == other.m_city && m_date == other.m_date &&
	       ...
	       m_humidity == other.m_humidity && m_pressure == other.m_pressure;
}
