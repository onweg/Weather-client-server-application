...
{
	executeAuthOperation(username, password,
	                     [this](const AuthorizationRequest &req)
...
void AuthViewModel::executeAuthOperation(const QString &username,
                                         const QString &password,
...
void AuthViewModel::setupWatcherAndStart(QFuture<AuthorizationReply> future)
{
	...
	connect(watcher, &QFutureWatcher<AuthorizationReply>::finished, this,
	        [this, watcher]() { handleAuthResult(watcher); });
	watcher->setFuture(future);
}

void AuthViewModel::handleAuthResult(
