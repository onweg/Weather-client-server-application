...
WeatherViewModel::WeatherViewModel(
    ...
    std::shared_ptr<SaveWeatherHistoryUseCase> saveHistory, QObject *parent)
    : QObject(parent),
      dailyUseCase_(std::move(dailyUC)),
      weeklyUseCase_(std::move(weeklyUC)),
...
void WeatherViewModel::clickSearchCityButton(const QString &city) {
  ...
  dailyUseCase_->execute(
      city.toStdString(),
      QDate::currentDate().toString("yyyy-MM-dd").toStdString(),
      [this](Result<WeatherData> result) {
        if (result.isSuccess()) {
        ...
        saveHistoryUseCase_->execute(desiredCity_.toStdString(),
                                     desiredDate_.toString().toStdString());
      });
}
...
void WeatherViewModel::clickNextDayButton() {
  ...
  dailyUseCase_->execute(
      desiredCity_.toStdString(), nextDate.toString("yyyy-MM-dd").toStdString(),
      [this](Result<WeatherData> result) {
        if (result.isSuccess()) {
          WeatherUiMapper::toUiModel(result.value(), weatherModel_);
        } else {
...
void WeatherViewModel::clickPrevDayButton() {
  ...
  dailyUseCase_->execute(
      desiredCity_.toStdString(), prevDate.toString("yyyy-MM-dd").toStdString(),
      [this](Result<WeatherData> result) {
        if (result.isSuccess()) {
          WeatherUiMapper::toUiModel(result.value(), weatherModel_);
        } else {
...
void WeatherViewModel::clickWeekWeatherDataButton() {
  weeklyUseCase_->execute(
      desiredCity_.toStdString(), [this](Result<WeekWeatherData> result) {
        if (result.isSuccess()) {
...
WeatherUiModel *WeatherViewModel::getWeatherModel() { return weatherModel_; }
