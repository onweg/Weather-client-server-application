...
WeatherViewModel::WeatherViewModel(
    ...
    QObject *parent)
    : QObject(parent), dailyUseCase_(std::move(dailyUC)),
...
void WeatherViewModel::clickSearchCityButton(const QString &city) {
    ...
    dailyUseCase_->execute(city.toStdString(),
...
void WeatherViewModel::clickNextDayButton() {
    ...
    if (!isDateValid(nextDate)) {
        qDebug() << "Дата выходит за пределы допустимого "
                    "диапазона";
        return;
    }
    desiredDate_ = nextDate;
    dailyUseCase_->execute(desiredCity_.toStdString(),
        nextDate.toString("yyyy-MM-dd").toStdString(),
        [this](Result<WeatherData> result) {
            if (result.isSuccess()) {
                WeatherUiMapper::toUiModel(result.value(), weatherModel_);
            } else {
...
void WeatherViewModel::clickPrevDayButton() {
    ...
    dailyUseCase_->execute(desiredCity_.toStdString(),
        prevDate.toString("yyyy-MM-dd").toStdString(),
        [this](Result<WeatherData> result) {
            if (result.isSuccess()) {
                WeatherUiMapper::toUiModel(result.value(), weatherModel_);
            } else {
...
void WeatherViewModel::clickWeekWeatherDataButton() {
    weeklyUseCase_->execute(
        desiredCity_.toStdString(), [this](Result<WeekWeatherData> result) {
            if (result.isSuccess()) {
                WeekWeatherUiMapper::toUiModel(
                    result.value(), weekWeatherModel_);
            } else {
