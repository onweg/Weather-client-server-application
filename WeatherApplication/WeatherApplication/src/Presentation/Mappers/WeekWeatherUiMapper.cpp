...
void WeekWeatherUiMapper::toUiModel(const WeekWeatherData &dto,
...
WeekWeatherData WeekWeatherUiMapper::fromUiModel(
    const WeekWeatherUiModel *uiModel) {
    if (!uiModel)
        return {};
    ...
    for (const auto *obj : uiModel->dailyWeather()) {
        ...
        if (weatherUi) {
            data.dailyWeather.push_back(
                WeatherUiMapper::fromUiModel(weatherUi));
        }
    }
