...
AuthorizationReply AuthorizationReplyJsonConverter::parseAuthorizationReply(
    const QJsonObject &jsonObj) {
    AuthorizationReply reply;
    if (!jsonObj.contains("status") || !jsonObj["status"].isString()) {
        reply.success = false;
        reply.message = "Invalid JSON: no 'status' field "
                        "or invalid type.";
        return reply;
    }
    ...
    if (!jsonObj.contains("message") || !jsonObj["message"].isString()) {
        reply.success = false;
        reply.message = "Invalid JSON: no 'message' field "
                        "or invalid type.";
        return reply;
    }
