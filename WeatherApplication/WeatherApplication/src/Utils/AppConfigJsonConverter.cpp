...
bool AppConfigJsonConverter::parseServerHostConfig(
    const QJsonObject& jsonObject, ServerHostConfig& config) {
  if (!jsonObject.contains("server host") ||
      !jsonObject["server host"].isObject()) {
    return false;
  }
...
bool AppConfigJsonConverter::parseApiConfig(const QJsonObject& jsonObject,
  ...
  if (!apiObj.contains("urlFindCityByName") ||
      !apiObj["urlFindCityByName"].isString()) {
    return false;
  }
  if (!apiObj.contains("urlFindWeatherByCoordinates") ||
      !apiObj["urlFindWeatherByCoordinates"].isString()) {
    return false;
  }
  ...
  config.urlFindCityByName =
      apiObj["urlFindCityByName"].toString().toStdString();
  ;
  config.urlFindWeatherByCoordinates =
      apiObj["urlFindWeatherByCoordinates"].toString().toStdString();
  ;
