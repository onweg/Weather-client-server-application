...
WeekWeatherData WeatherJsonConverter::parseWeekWeather(
    ...
    for (const QJsonValue &forecastValue : forecasts) {
        ...
        QDateTime forecastTime =
            QDateTime::fromString(dtTxt, "yyyy-MM-dd HH:mm:ss");
        QString dateKey = forecastTime.date().toString("yyyy-MM-dd");
        if (!forecast.contains("main") || !forecast.contains("weather"))
            continue;
        QJsonObject mainData = forecast["main"].toObject();
        QJsonArray weatherArray = forecast["weather"].toArray();
        QJsonObject windData = forecast.contains("wind")
                                   ? forecast["wind"].toObject()
                                   : QJsonObject();
        WeatherData data;
        ...
        if (forecastTime.date() == currentDate) {
            if (hour >= 12) {
                ...
                if (dailyData.contains(dateKey)) {
                    QTime storedTime = QDateTime::fromString(
                        QString::fromStdString(dailyData[dateKey].date),
                        "yyyy-MM-dd")
                                           .time();
                ...
                if (!dailyData.contains(dateKey) ||
                    (currentHourDiff < storedHourDiff)) {
