...
WeekWeatherData WeatherJsonConverter::parseWeekWeather(const QJsonObject& jsonObj) {
    ...
    QMap<QString, WeatherData> dailyData = groupForecastsByDay(forecasts, cityName);
...
QMap<QString, WeatherData> WeatherJsonConverter::groupForecastsByDay(
        const QJsonArray& forecasts, const QString& cityName)
{
    QMap<QString, WeatherData> dailyData;
    QDate currentDate = QDate::currentDate();
    for (const QJsonValue& forecastValue : forecasts) {
        if (!forecastValue.isObject()) continue;
        processForecast(forecastValue.toObject(), cityName, currentDate, dailyData);
    }
    return dailyData;
}
...
void WeatherJsonConverter::processForecast(
        ...
        const QDate& currentDate, QMap<QString, WeatherData>& dailyData)
{
    QDateTime forecastTime = parseForecastTime(forecast);
    if (!forecastTime.isValid()) return;
    QString dateKey = forecastTime.date().toString("yyyy-MM-dd");
    WeatherData data = parseWeatherData(forecast, cityName, forecastTime);
    selectBestForecastForDay(data, dateKey, forecastTime, currentDate, dailyData);
}
...
WeatherData WeatherJsonConverter::parseWeatherData(
        const QJsonObject& forecast, const QString& cityName, const QDateTime& forecastTime)
{
    ...
    if (forecast.contains("weather") && forecast["weather"].isArray()) {
        ...
        if (!weatherArray.isEmpty()) {
            data.description = weatherArray.first().toObject()["description"].toString().toStdString();
        }
    }
    return data;
}
...
void WeatherJsonConverter::selectBestForecastForDay(
        const WeatherData& data, const QString& dateKey,
        ...
        QMap<QString, WeatherData>& dailyData)
{
    ...
    if (forecastTime.date() == currentDate && hour >= 12) {
        if (!dailyData.contains(dateKey) || isBetterMatch(hour, dailyData, dateKey)) {
...
bool WeatherJsonConverter::isBetterMatch(int currentHour, const QMap<QString, WeatherData>& dailyData, const QString& dateKey) {
...
QStringList WeatherJsonConverter::sortedKeys(const QMap<QString, WeatherData>& dailyData) {
